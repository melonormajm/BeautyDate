 ...
 * @since 2.0
 */
class RestAuth extends ActionFilter
{
    /**
    ...
    {
        //echo '<pre>'; print_r($request->queryParams); die;
        $this->user->enableSession = false;
        $this->user->loginByAccessToken(Yii::$app->getRequest()->get('token'));

        if ($this->user->isGuest) {
            throw new ForbiddenHttpException();
            return false;
        }
            //throw new ResultException(ResultException::CODE_ERROR, ResultException::MSG_AUTH_ERROR);
