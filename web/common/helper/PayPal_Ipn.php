...
class PayPal_Ipn
{
    ...
    function init()
    {
        ...
        if ($ch == FALSE) {
            ...
            Yii::error("Can't connect to PayPal to validate IPN message: " . curl_error($ch), 'paypal');
            return FALSE;
        }
        ...
        if ($this->debug == TRUE) {
           // error_log(PHP_EOL . date('[Y-m-d H:i e] ') . "HTTP request of validation request:" . curl_getinfo($ch, CURLINFO_HEADER_OUT) . PHP_EOL, 3, $this->log);
            Yii::error("HTTP request of validation request:" . curl_getinfo($ch, CURLINFO_HEADER_OUT), 'paypal');
            //error_log(PHP_EOL . date('[Y-m-d H:i e] ') . "HTTP response of validation request: " . $this->validate . PHP_EOL, 3, $this->log);
            Yii::error("HTTP response of validation request: " . $this->validate, 'paypal');
        }
        ...
        if (stripos($return, 'VERIFIED') !== FALSE) {
            ...
            Yii::error('VERIFIED: ', 'paypal');
            Yii::error( print_r($this->validate, TRUE) , 'paypal');
            return TRUE;
        }

        //error_log(PHP_EOL . date('[Y-m-d H:i e] ') . "Invalid IPN:" . $this->validate . PHP_EOL, 3, $this->log);
        Yii::error( "Invalid IPN:" . $this->validate , 'paypal');
        return FALSE;
    }
